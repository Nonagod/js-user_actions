...
async function getObjectFingerprint(object) {
  object = (0, import_lodash.default)(object);
  if (object.data) {
    const entries = [];
...
// src/RequestsEmulator.js
var RequestsEmulator = class {
  _emulated_requests = {};
  constructor() {
  }
...
var UserAction = class {
  ...
  do(action, options = {}) {
    try {
      options = resolveOptions_default(action, options);
    } catch (Error2) {
      console.error(Error2);
    } finally {
      let UserRequest = new Request(options);
      UserRequest.getFingerprint().then((fingerprint) => {
  ...
  _handleError(UserRequest, Error2) {
    if (!Boolean(Error2 instanceof ServerRespondError)) Error2 = new SomethingWentWrongError(Error2);
    (UserRequest.options.handleError ?? this.handleErrorDefault)(Error2);
  }
};
